    ...
    tokens.map(async (token: any) => {
      try {
        ...
        const balances = await Promise.all(
          wallets.map((w: any) => getTokenBalance(w.address, token.mint))
        );
        ...
        let currentPrice = 0;
        let priceUsd = 0;
        const priceData = await pumpPortalAPI.getTokenData(token.mint);
        ...
        const buyEvents = await prisma.eventLog.findMany({
          where: {
            type: "TOKEN_BUY",
            mint: token.mint,
          },
        ...
        const invested = buyEvents.reduce((sum: number, event: any) => {
...
export function calculatePnL(currentPrice: number, invested: number, balance: number) {
...
export async function migrateOldTokens() {
  try {
    // Find all tokens where ownerWallet doesn't look like a valid Solana address
    const allTokens = await prisma.token.findMany();
    
    const tokensToFix = allTokens.filter((token: any) => {
