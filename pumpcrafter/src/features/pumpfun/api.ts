...
import { Keypair, VersionedTransaction, Connection, sendAndConfirmRawTransaction } from "@solana/web3.js";
...
class PumpPortalAPI {
  private tradeLocalUrl = "https://pumpportal.fun/api/trade-local";
  private ipfsUploadUrl = "https://pump.fun/api/ipfs";
  ...
  async uploadMetadataToIPFS(params: {
    ...
    try {
      ...
      const response = await axios.post(this.ipfsUploadUrl, formData, {
  ...
  async generateBundledTransactions(
  ...
  async signAndSendTransaction(
    ...
    try {
      ...
      try {
        ...
        transactionBuf = new Uint8Array(bs58.decode(encodedTransaction));
      ...
      try {
        ...
        const transaction = VersionedTransaction.deserialize(new Uint8Array(transactionBuf));
  ...
  async sendJitoBundle(
    encodedSignedTransactions: string[]
  ...
  async sendTransactionsSequentially(
    ...
    try {
      ...
      for (let i = 0; i < encodedSignedTransactions.length; i++) {
        try {
          const txBuf = bs58.decode(encodedSignedTransactions[i]!);
  ...
  private async waitForBundleConfirmation(bundleId: string, maxAttempts = 15): Promise<boolean> {
