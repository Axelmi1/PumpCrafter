...
export function viewProjectsList(projects: Array<{
  id: string; projectId: string; name?: string|null; symbol?: string|null; 
  status: string; createdAt: Date; bundleCount: number;
}>) {
...
export function viewProjectDetails(
  ...
  const fundedWallets = project.projectWallets?.filter((pw: any) => pw.isFunded).length || 0;
...
export function viewProjectType() {
  const text =
`🎯 Choose Project Type
...
export function viewMetadata(d: {
  id?: string;
  name?: string|null; symbol?: string|null; description?: string|null;
  twitter?: string|null; telegram?: string|null; website?: string|null; imageFileId?: string|null;
}) {
  const text =
`🎯 Project Metadata
...
export function viewBundleConfig(project: {
  id: string; bundleCount: number; buyAmountPerWallet: number;
}) {
  const totalSOL = project.bundleCount * project.buyAmountPerWallet;
...
export function viewWalletsList(wallets: Array<{
  id: string; address: string; label?: string|null; balance: number; isCreator: boolean;
}>) {
...
export function viewWalletDetails(wallet: {
  id: string; address: string; label?: string|null; balance: number; isCreator: boolean;
}) {
...
export function viewWalletSelection(
  wallets: Array<{
    id: string; address: string; label?: string|null; balance: number;
  }>, 
  selectedIds: string[], 
  projectId: string,
...
export function viewPortfolio(positions: Array<{
  token: { id: string; mint: string; name: string; symbol: string; pumpfunUrl?: string|null };
  totalBalance: number;
  currentPrice: number;
  priceUsd?: number;
  currentValue: number;
...
export function viewLaunchSummary(
  project: {
    id: string;
    name?: string|null;
    symbol?: string|null;
    bundleCount: number;
    buyAmountPerWallet: number;
    projectWallets?: Array<{
      wallet: { address: string; label?: string|null; balance?: number };
      buyAmount: number;
      isFunded: boolean;
    }>;
  },
  ...
  const bundleWallets = (project.projectWallets || []).filter((pw: any) => {
...
${creatorStatus} Creator: ${creatorWallet.balance.toFixed(4)} SOL (needs ${creatorNeeded.toFixed(4)} SOL)`;
  ...
  if (bundleWallets.length > 0) {
    bundleWallets.forEach((pw: any, i: number) => {
...
export function viewTokenPosition(position: {
  token: { mint: string; name: string; symbol: string; pumpfunUrl?: string|null };
  totalBalance: number;
  currentPrice: number;
  priceUsd?: number;
  currentValue: number;
...
export function viewTokenTrading(position: {
  token: { mint: string; name: string; symbol: string; pumpfunUrl?: string|null };
  totalBalance: number;
  currentPrice: number;
  priceUsd?: number;
